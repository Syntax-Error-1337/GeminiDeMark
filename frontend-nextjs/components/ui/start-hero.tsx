...
export function StartHero({ onFilesSelected }: StartHeroProps) {
    ...
    return (
        <section className="relative pt-16 pb-12 lg:pt-24 lg:pb-20 text-center px-4 overflow-hidden">
            <h2 className="hero-title font-extrabold text-white mb-6 md:text-6xl text-4xl tracking-tight drop-shadow-2xl">
                {t('main.title') || 'Gemini AI Watermark Removal'}
            </h2>
            <p className="hero-subtitle text-base md:text-lg text-slate-200/80 max-w-2xl mx-auto mb-12 font-medium tracking-wide">
            ...
            <div className="max-w-6xl mx-auto px-4 mt-24 mb-16 md:mb-24 relative z-10">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative">
                    ...
                    <div className="hidden md:block absolute top-1/2 left-0 w-full h-1 bg-white/10 -translate-y-1/2 -z-10 rounded-full"></div>

                    {steps.map((step) => (
                        <div
                            key={step.id}
                            className={cn(
                                "group relative p-6 rounded-2xl border shadow-lg transition-all duration-300 flex items-center gap-5 z-10 hover:-translate-y-1 backdrop-blur-sm",
                                step.className,
                                step.id === 2 ? "md:scale-110 md:shadow-2xl" : ""
                            )}
                        >
                            <div className="w-12 h-12 rounded-xl bg-white/20 flex items-center justify-center font-bold text-lg text-white border border-white/20 shadow-inner">
                                {step.id}
                            </div>
                            <div className="flex flex-col text-left">
                                <span className="material-icons-round text-2xl text-white/90 mb-1 group-hover:scale-110 transition-transform origin-left">
                                    {step.icon}
                                </span>
                                <span className="font-bold text-white text-lg tracking-wide leading-tight">{step.title}</span>
                            </div>
                        </div>
                    ))}
                </div>
